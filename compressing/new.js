...
async function compress(compression) {
  return new Promise(function (resolve, reject) {
    ...
    for (let i = 0; i < filelist.length; i++) {
      ...
      if (filelist[i].endsWith('dlog.txt')) { // size issue
        fileName = filelist[i]
      }
    ...
    let compressionFile = ''
    if (fileName === '') {
      compressionFile = path.join(pathName_ZIP, compression) + '.zip'
    } else {
...
let Task = async () => {
  ...
  ************************************************** `)
  ...
  console.time('start');
  for (let index = 0; index < Files.length; index++) {
    let re = await compress(Files[index])
    if (!(re === 'nullFolder')) {
      await filesOperation(Files[index])
    }
