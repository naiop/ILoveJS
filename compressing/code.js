...
let WinRARPath = "D:\\rar软件\\WinRAR.exe" //压缩软件WinRAR存放路径
let pathName = "D:\\workSpace\\renameFile\\data" //原数据储存路径
let pathName_ZIP = "D:\\workSpace\\renameFile\\zip"  //最后压缩 所有文件的路径
let pathName_backup = "D:\\workSpace\\renameFile\\bak"  //解压过的文件存放路径
let TmpDir = "C:\\ZIP"  //解压过的文件存放路径
...
init() // 初始化文件夹，没有就创建
  ...
  for (let index = 0; index < num.length; index++) {
    ...
    if (index+1===num.length) {
      console.log(`总共有${num.length}文件,压缩成功了${ File.length }`); 

      for (let index = 0; index < num.length; index++) {
        const element = num[index];
        await copyDirectory(pathName +'\\'+element , pathName_backup+ "\\"+ (pathName +'\\'+element).substring((pathName +'\\'+element).lastIndexOf('\\'),(pathName +'\\'+element).length) )
      }
 ...
 * logger('err', 'log');
 * @param {*} data  日志信息
 * @param  {...any} args  [0] 文件名
 */
function logger(data, ...args) {
	let tData="";
	if(data instanceof Error){
		tData=`${data.name?(data.name+"\n"):""}${data.data?(data.data+"\n"):""}${data.stack?(data.stack+"\n"):""}`;
	}else{
		tData= JSON.stringify(data,null,"\t")+"\n";
	}
    let arg = "";
    if(true){
        let logDir = `${TmpDir}/${args[0]}/logs`;
        mkdirsSync(logDir);
        fs.writeFileSync(`${logDir}/${args[0]}-${moment().format('YYYY-MM-DD')}.log`, `${moment().format('YYYY-MM-DD HH:mm:ss')} : ${tData}`,{ encoding: 'utf8', mode: 438 /*=0666*/, flag: 'as' });
    }
}